...
build_rotation_schedules_old <- function(){
  ...
  if(m_yr_sch_existing == 'y'){
    ...
    for(k in 1:length(idz)){
      mgt_crop_sub <- mgt_crop[which(mgt_crop$crop_mgt== crop_mgt.list[idz[k]]),]
      yr <- unlist(strsplit(mgt_crop_sub$crop_mgt[1],"\\."))[1]
      if(substr(yr,nchar(yr),nchar(yr))=='1') mgt_crop_sub$crop_mgt <- paste0(mgt_crop_sub$crop_mgt,'_1')
      if(substr(yr,nchar(yr),nchar(yr))=='2') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:length(mgt_crop_sub$crop_mgt))
        mgt_crop_sub$crop_mgt[idx1] <- paste0(mgt_crop_sub$crop_mgt[idx1],'_1')
      ...
      if(substr(yr,nchar(yr),nchar(yr))=='3') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:(pos_skip_[3]-1))
        idx3 <- c(pos_skip_[3]:length(mgt_crop_sub$crop_mgt))
      ...
      if(substr(yr,nchar(yr),nchar(yr))=='4') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:(pos_skip_[3]-1))
        idx3 <- c(pos_skip_[3]:(pos_skip_[4]-1))
      ...
      if(substr(yr,nchar(yr),nchar(yr))=='5') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:(pos_skip_[3]-1))
        idx3 <- c(pos_skip_[3]:(pos_skip_[4]-1))
      ...
      if(substr(yr,nchar(yr),nchar(yr))=='6') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:(pos_skip_[3]-1))
        idx3 <- c(pos_skip_[3]:(pos_skip_[4]-1))
      ...
      if(substr(yr,nchar(yr),nchar(yr))=='7') {
        pos_skip_ <- grep("skip", mgt_crop_sub$operation)
        idx1 <- c(1:(pos_skip_[2]-1))
        idx2 <- c(pos_skip_[2]:(pos_skip_[3]-1))
        idx3 <- c(pos_skip_[3]:(pos_skip_[4]-1))
  ...
  for(m in 1:length(lu.list)){
    ...
    if(m_yr_sch_existing == 'y'){
      if(crop_myr %in% substr(rota,1,4)){
        ...
        if(all(substr(rota[pos_scrop],1,4)%in%crop_s & length(pos_scrop)==1)){
          rota[which(myr_seq<pos_scrop)] <- paste0(rota[which(myr_seq<pos_scrop)],'_',myr_max[which(myr_seq<pos_scrop)],'.5yr','_',myr_idx[which(myr_seq<pos_scrop)])
          if(additional_h_yr_sch_existing == 'y') rota[pos_scrop] <- paste0(rota[pos_scrop],'_0.5yr')
        }
        if(length(pos_scrop)==2){
          if(substr(rota[pos_scrop[1]],1,4)%in%crop_s){
            rota[which(myr_seq<pos_scrop[1])] <- paste0(rota[which(myr_seq<pos_scrop[1])],'_',myr_max[which(myr_seq<pos_scrop[1])],'.5yr','_',myr_idx[which(myr_seq<pos_scrop[1])])
            if(additional_h_yr_sch_existing == 'y') rota[pos_scrop][1] <- paste0(rota[pos_scrop][1],'_0.5yr')
          }
          if(substr(rota[pos_scrop[2]],1,4)%in%crop_s){
            rota[which(myr_seq>pos_scrop[1] & myr_seq<pos_scrop[2])] <- paste0(rota[which(myr_seq>pos_scrop[1] & myr_seq<pos_scrop[2])],'_',myr_max[which(myr_seq>pos_scrop[1] & myr_seq<pos_scrop[2])],'.5yr','_',myr_idx[which(myr_seq>pos_scrop[1] & myr_seq<pos_scrop[2])])
            if(additional_h_yr_sch_existing == 'y') rota[pos_scrop][2] <- paste0(rota[pos_scrop][2],'_0.5yr')
          }
        }
        if(length(pos_scrop)==3){
          ...
          if(substr(rota[pos_scrop[3]],1,4)%in%crop_s){
            rota[which(myr_seq>pos_scrop[2] & myr_seq<pos_scrop[3])] <- paste0(rota[which(myr_seq>pos_scrop[2] & myr_seq<pos_scrop[3])],'_',myr_max[which(myr_seq>pos_scrop[2] & myr_seq<pos_scrop[3])],'.5yr','_',myr_idx[which(myr_seq>pos_scrop[2] & myr_seq<pos_scrop[3])])
            if(additional_h_yr_sch_existing == 'y') rota[pos_scrop][3] <- paste0(rota[pos_scrop][3],'_0.5yr')
          }
        }
